...
- Match -> semantic proxy score -> blended score -> rank -> package -> guardrails
...
- Output: match artifact + `match_score` + `semantic_score` + `blended_score`
...
- blended ranking signal (keyword + semantic proxy) for better relevance
