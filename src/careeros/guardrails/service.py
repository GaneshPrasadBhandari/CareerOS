...
#dynamic guardrail loading and execution framework (future work)
from __future__ import annotations
...
# Token pattern: matches words and technical tools (fastapi, scikit-learn, etc.)
TOKEN_RE = re.compile(r"[A-Za-z][A-Za-z0-9\-\+\.]{1,40}")
...
def validate_package_against_evidence(
    ...
    """
    RESOLVED GUARDRAIL:
    1. Uses a Watchlist to identify 'Technical Tools' (to avoid blocking common words).
    2. Compares tools in the generated package against user's EvidenceProfile.
    3. If a 100% match or similar is detected, it issues a WARNING instead of a BLOCK.
    """
    evidence_skills = {s.lower().strip() for s in (profile.skills or []) if s and s.strip()}

    # High-signal tool watchlist (Prevents false positives on words like 'the', 'manager', etc.)
    watchlist = {
        "python", "sql", "docker", "kubernetes", "fastapi", "streamlit", "pydantic",
        "pytest", "git", "linux", "mlflow", "dvc", "airflow", "terraform", "kafka",
        "spark", "databricks", "snowflake", "aws", "azure", "gcp", "sagemaker",
        "llm", "genai", "rag", "langchain", "langgraph", "pytorch", "tensorflow"
    }

    # Collect all text from bullets and cover letter
    segments: List[str] = []
    bullets = getattr(pkg, "bullets", []) or []
    for b in bullets:
        segments.append(getattr(b, "text", "") or "")
    ...
    # Find technical tools mentioned in the AI output that the user DOES NOT have
    unsupported_set: Set[str] = set()
    for seg in segments:
        for tok in TOKEN_RE.findall(seg):
            t = tok.lower().strip()
            # If the word is a known Technical Tool AND not in user skills -> Flag it
            if t in watchlist and t not in evidence_skills:
                unsupported_set.add(t)
