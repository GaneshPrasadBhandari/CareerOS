...
def _semantic_similarity_score(resume_text: str, job_text: str) -> float:
    """Lightweight semantic proxy score (no paid APIs/models required).

    Combines token Jaccard overlap and sequence similarity to provide a more stable
    relevance estimate than strict keyword-only matching.
    """
    rt = _tokenize_text(resume_text)
    jt = _tokenize_text(job_text)
    if not rt or not jt:
        return 0.0

    rs, js = set(rt), set(jt)
    jaccard = len(rs & js) / max(1, len(rs | js))
    seq = SequenceMatcher(None, " ".join(rt[:1200]), " ".join(jt[:1200])).ratio()
...
def p25_automation_run(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    semantic_score = _semantic_similarity_score(resume_text, job_obj.get("raw_text", ""))
    blended_score = round((0.65 * match.score) + (0.35 * semantic_score), 2)
    ...
    trace = {
        ...
        "layers": {
            ...
            "L4_matching": {
                ...
                "output": {"match_path": match_path, "match_score": match.score, "semantic_score": semantic_score, "blended_score": blended_score},
